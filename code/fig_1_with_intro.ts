...
let animation_state: { t: number, state: SokobanState, pos_x: number, pos_y: number, node: Node, view_x: number, view_y: number } | null = null;
...
network.on("afterDrawing", function (ctx: CanvasRenderingContext2D) {
    if (animation_state !== null) return;
...
function expandNode(node: Node): boolean {
    ...
    for (const [input, new_state] of Object.entries(next_states)) {
        ...
        if (nodes.get(new_id) === null) {
            addNode(new_state as SokobanState);
            /*nodes.add({
                id: new_id,
                state: new_state,
                won: new_state.isWon(),
...
function addMissingEdges(new_node: Node): void {
    // new_node is source
    ...
    for (const source_node of nodes.get()) {
        if (source_node.id === new_node.id) continue;
        let next_states = State.nextStates(source_node.state, modulo_player, see_clearly);
        for (const [input, new_state] of Object.entries(next_states)) {
            if (new_node.id !== State.id(new_state)) continue;
...
function addNode(state: SokobanState, x: number | undefined = undefined, y: number | undefined = undefined) {
...
function otherSetNode(state: SokobanState) {
