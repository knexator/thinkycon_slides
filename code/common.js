...
function nextStates(state, modulo, seeClearly) {
    if (modulo) {
...
function nextStates_modulo(state, seeClearly) {
    let result = {};
    let explored = rectGrid(false, state.w, state.h);
    let pending = [state.player];
    while (pending.length > 0) {
...
function nextStates_full(state, seeClearly) {
    let result = {
...
function drawState(state, ctx, pos, main = false) {
    let TILE_S = main ? ctx.canvas.height / state.h : 50;
...
export function rotVec(a, radians) {
...
export let State = { initialState, nextStates, isWon, id, isClearlyLost, drawState, /*drawToUrl*/ };
